...
const result = data.map((user) => ({ id: user.id, username: user.username }));
console.log(result);
...
const userNine = data.find((user) => user.id === 9);
console.log(userNine);

// 4. Вывести всех пользователей из города 'Lebsackbury'

const usersCity = data.find((user) => user.address.city === "Lebsackbury");
...
function checkWebsite(website) {
  return website.match(/\.com$/);
}
const websiteUsers = data.filter((person) => checkWebsite(person.website));
...
const longUsernames = data.filter((person) => checkLength(person.username));
...
const lowerCaseUsernames = data.map((person) => toLowerCase(person.username));
